...
@export var max_step_distance : float = 30.0
...
func step():
	...
	new_tween.tween_property(self, "global_position", Vector2(half_point.x + (step_height.x/4) * owner.current_h_facing, half_point.y + step_height.y), 0.1)
	new_tween.tween_property(self, "global_position", Vector2(target_pos.x + (step_height.x)/2 * owner.current_h_facing, target_pos.y), 0.1)
